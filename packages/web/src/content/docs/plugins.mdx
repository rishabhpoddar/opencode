...
### Rewrite shell commands before execution

Rewrite the command or args that are about to be launched, without changing the chat-visible tool history:

```javascript title=".opencode/plugins/rewrite-shell.js"
export const RewriteShellPlugin = async () => {
  return {
    "shell.command.before": async (input, output) => {
      if (input.command.includes("{{SECRET:")) {
        output.command = input.command.replaceAll("{{SECRET:API_KEY}}", "${__TEAMCOPILOT_RUNTIME_SECRET_API_KEY}")
      }
    },
  }
}
